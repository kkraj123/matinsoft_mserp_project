...
class StartSurveyView extends StatefulWidget {
  ...
  const StartSurveyView({super.key, required this.surveyObj, this.existingResponse,
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return SurveyQuestionWidget(
                question: question,
                // ✅ FIXED: Check both maps for current answer
                currentAnswer:
                    answers[question.questionId] ??
                    filePaths[question.questionId],
                onChanged: (value) {
                  setState(() {
                    // ✅ FIXED: Separate file and regular answers
                    if (question.type == 'file' || question.type == 'image') {
                      // For files: only store in filePaths, NOT in answers
                      filePaths[question.questionId] = value;
                      answers.remove(question.questionId); // Remove if exists
                    } else {
                      // For regular answers: only store in answers
          ...
          decoration: InputDecoration(
            ...
            focusedBorder: OutlineInputBorder(
              borderSide: BorderSide(
                color: themeProvider.isDarkMode
                    ? Colors
                          .amber // focused color for dark mode
                    : AppColors.primaryColor, // focused color for light mode
                width: 2,
              ),
              borderRadius: BorderRadius.circular(8),
            ),
        ...
        InkWell(
          onTap: () async {
            ...
            if (result != null && result.files.isNotEmpty) {
              ...
              onChanged(filePath); // <<< ✔ Pass to parent
